...
export async function fetchLongQuests(userId: string): Promise<LongQuest[]> {
  ...
  const { data: stages, error: stagesError } = await supabase
    ...
    .select('id, long_quest_id, name, done, position')
    .in(
      'long_quest_id',
      quests.map(q => q.id)
    )
  ...
  for (const s of stages ?? []) {
    ...
    stagesByQuest.get(s.long_quest_id)!.push({ id: s.id, name: s.name, done: s.done });
  }
...
export async function createLongQuest(userId: string, input: LongQuestInput): Promise<string> {
  const { data: quest, error } = await supabase
    .from('long_quests')
    .insert({ user_id: userId, name: input.name, stat: input.stat })
    .select('id')
    .single();
  if (error) throw error;
